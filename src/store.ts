...
export class EvidenceStore {
  ...
  addPage(input: { platform: string; handle: string; caseId?: string; reason?: string }): PageRecord {
  ...
  duePages(caseId?: string): PageRecord[] {
    const sql = caseId
      ...
      : `SELECT * FROM pages WHERE stance != 'muted'
         ORDER BY COALESCE(last_crawled_at, '') ASC`;
    const rows = (caseId
      ? this.db.prepare(sql).all(caseId)
  ...
  recordPost(post: Omit<StoredPost, "id"> & { id?: string }): string {
    ...
    this.db
      ...
      .run(
        postId, post.pageId, post.caseId, post.platform, post.platformPostId,
        post.url, post.contentText, post.authorName, post.postedAt, now(),
      );
  ...
  recordComment(input: {
    ...
    this.db
      ...
      .run(commentId, input.postId, input.platformCommentId, input.text,
           input.authorName ?? null, input.postedAt ?? null, now());
  ...
  recordVerdict(v: StoredVerdict): string {
    ...
    this.db
      ...
      .run(
        verdictId, v.postId ?? null, v.commentId ?? null, v.verdict,
        v.hateSpeechFlag ? 1 : 0, v.confidence, v.category ?? null, v.severity ?? null,
        v.targetGroupSlug ?? null, v.reliesOnContext ? 1 : 0,
        v.committeeDisagreement ? 1 : 0, v.rationale ?? null,
        JSON.stringify({
  ...
  recordReview(input: {
    ...
    this.db
      ...
      .run(id(), input.verdictId, input.reviewerId, input.confirmed ? 1 : 0,
           input.rationale ?? null, now());
  }
  ...
  recordCrawl(input: {
    ...
    this.db
      ...
      .run(id(), input.pageId, input.caseId ?? null, now(), now(),
           input.postsSeen, input.commentsSeen, input.flagged,
           input.baseline ? 1 : 0, input.error ?? null);
  ...
  pendingOutbox(limit = 50): Array<{ id: string; kind: string; payload: unknown; requestId: string }> {
    const rows = this.db
      .prepare(

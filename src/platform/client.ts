...
export class PlatformClient {
  ...
  async getTropes(targetGroups: readonly string[] = []): Promise<{ tropes: PlatformTrope[]; total: number }> {
  ...
  ): Promise<{ accepted: number; gap_ids: number[]; duplicates: string[]; rejected: Array<{ term: string; reason: string }> }> {
  ...
  async postScanLog(payload: Record<string, unknown>, idempotencyKey: string): Promise<{ id: number }> {
  ...
  private async request<T>(
    ...
    if (response.status === 401 || response.status === 403) {
      throw new AgentKeyRejected(detail || `agent key rejected (${response.status})`, response.status);
    }
    ...
    throw new PlatformError(detail || `request failed (${response.status})`, response.status, retryable);
  }
}

...
const LEXICON = {
  terms: [{
    id: 1, term: "نجس", language: "ar", category: "dehumanization",
    target_group: "Yazidi", target_group_slug: "yazidi", severity_weight: 8,
    is_regex: false, is_explicit: true,
    never_flag_when: ["academic", "news_quotation", "counter_speech"], variants: ["انجاس"],
  }],
  tropes: [{
    id: 1, name: "Devil-worship libel",
    description: "Invokes the devil-worship libel", example: null,
    target_group: null, target_group_slug: "yazidi",
    target_groups: ["yazidi", "christian-iraqi"],
    activation_topics: ["ايزيدي", "لالش"],
    surface_forms: ["اعوذ بالله من الشيطان الرجيم", "عبدة الشيطان"],
    requires_target_group: true, negation_cancels: true,
    negative_examples: [], counter_speech_examples: [],
    severity_weight: 8, is_visual: false,
  }],
  total: 1,
};

const server = createServer((req, res) => {
  if (!req.headers.authorization?.startsWith("Bearer ")) { res.writeHead(401).end("{}"); return; }
  if (!req.headers["x-agent-id"]) { res.writeHead(400).end('{"error":"missing X-Agent-Id"}'); return; }
...
const call = (name, params = {}) =>
  byName[name].execute("call-1", params).then((r) => r.details);
...
const config = {
  platformUrl: `http://127.0.0.1:${port}/`,
  agentKey: "test-key", agentId: "ankedo-e2e",
  databasePath: ":memory:", maxStaleHours: 6,
};


const checks = [];
const check = async (n, f) => { try { await f(); checks.push(["ok  ", n]); } catch (e) { checks.push(["FAIL", `${n}: ${e.message}`]); } };
...
await check("hate: نجس about people under a Yazidi post", async () => {
  const r = await call("classify", { text: "هذوله نجس وما يصير تاكل من ايدهم", parentPostText: LALISH, targetGroups: ["yazidi"] });
  assert.equal(r.verdict, "hate", JSON.stringify(r));
  assert.equal(r.flagged, true);
  assert.equal(r.judgedInContext, true);
  assert.deepEqual(r.lexiconHits.map(h => h.matched), ["نجس"]);
});

await check("benign: the same word about water", async () => {
  const r = await call("classify", { text: "الماي وصلنا نجس، ما ينشرب أبدا", parentPostText: WATER, targetGroups: [] });
...
await check("trope fires only with the group in context", async () => {
  const hit = await call("classify", { text: "اعوذ بالله من الشيطان الرجيم", parentPostText: LALISH, targetGroups: ["yazidi"] });
  assert.equal(hit.tropesFired.length, 1, "the libel did not fire under a Lalish post");

  const miss = await call("classify", { text: "اعوذ بالله من الشيطان الرجيم", parentPostText: "الطقس حار اليوم", targetGroups: [] });
...
await check("counter-speech is not treated as an attack", async () => {
  const r = await call("classify", { text: "يسمونهم عبدة الشيطان وهذا افتراء", parentPostText: LALISH, targetGroups: ["yazidi"] });
...
await check("a case can be opened and reports its schedule", async () => {
  const opened = await call("case_open", { targetGroupSlug: "yazidi", narrative: "devil-worship libel" });
  assert.ok(opened.id);
  const status = await call("case_status", {});
  assert.equal(status.cases.length, 1);
  assert.equal(status.cases[0].schedule.learned, false, "should not claim to have learned from nothing");

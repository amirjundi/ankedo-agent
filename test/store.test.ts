...
describe("verdicts", () => {
  it("keeps the whole trace", () => {
    ...
    s.recordVerdict({
      ...
      tropesFired: [{ trope_id: 1, name: "Devil-worship libel", surface_form: "x",
                      matched_group: "yazidi", covers_groups: ["yazidi"], severity: 8,
                      implicature: "…", reason: "activation satisfied" }],
      lexiconAgeHours: 3.5,
    });
  ...
  it("records how stale the lexicon was", () => {
    ...
    const s = store();
    const p = s.recordPost(post());
    s.recordVerdict({ postId: p, verdict: "hate", hateSpeechFlag: true,
                      confidence: 0.9, lexiconAgeHours: 168 });
...
describe("review queue", () => {
  it("holds flagged, ambiguous and disagreed items", () => {
    ...
    s.recordVerdict({ postId: p, verdict: "benign", hateSpeechFlag: false,
                      confidence: 0.9, committeeDisagreement: true });
...
describe("what the schedule learns from", () => {
  it("reports the hour content was posted, not collected", () => {
    ...
    const c = s.openCase({ targetGroupSlug: "yazidi" });
    for (let i = 0; i < 3; i++) {
      const p = s.recordPost(post({ caseId: c.id, platformPostId: `p${i}`,
                                    postedAt: "2026-08-28T21:30:00Z" }));
      s.recordVerdict({ postId: p, verdict: "hate", hateSpeechFlag: true, confidence: 0.9 });
    }
  ...
  it("feeds the window calculation end to end", () => {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const p = s.recordPost(post({
        caseId: c.id, platformPostId: `p${i}`,
  ...
  it("feeds the page stance decision end to end", () => {
    ...
    for (let i = 0; i < 30; i++) {
      s.recordCrawl({ pageId: quiet.id, postsSeen: 5, commentsSeen: 20, flagged: 0, baseline: false });
    }
